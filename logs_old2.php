...
{
    $query = "SELECT * FROM ( SELECT LOGID,TERMNO,PINSERIAL,TO_CHAR(UNIXTS_TO_DATETIME(UPDATETIME),'YYYY-MM-DD HH24:MI:SS'),CARDNO,CATEGORY,TRACENO,RESPONSECODE,SUBSTR(ORIGINALDATA,5,6),ACTIONID,ROWNUM AS RNUM from ( select * from tbllogs order by logid desc ) ) WHERE RNUM>{$start} and RNUM<{$end}";
}
else if ( $trace != "" )
{
    $query = "SELECT * FROM ( SELECT LOGID,TERMNO,PINSERIAL,TO_CHAR(UNIXTS_TO_DATETIME(UPDATETIME),'YYYY-MM-DD HH24:MI:SS'),CARDNO,CATEGORY,TRACENO,RESPONSECODE,SUBSTR(ORIGINALDATA,5,6),ACTIONID,ROWNUM AS RNUM FROM ( select * from tbllogs WHERE TRACENO='".$trace."' OR SUBSTR(ORIGINALDATA,5,6)='".$trace."' ORDER BY LOGID DESC)) WHERE RNUM>{$start} and RNUM<{$end}";
}
else if ( $term != "" )
{
    if ( isset( $_GET['Date'] ) && $_GET['Date'] != "" )
    {
        $query = "SELECT * FROM ( SELECT LOGID,TERMNO,PINSERIAL,TO_CHAR(UNIXTS_TO_DATETIME(UPDATETIME),'YYYY-MM-DD HH24:MI:SS'),CARDNO,CATEGORY,TRACENO,RESPONSECODE,SUBSTR(ORIGINALDATA,5,6),ACTIONID,ROWNUM AS RNUM  FROM ( select * from tbllogs WHERE TBLLOGS.TERMNO='".$term."' AND TO_CHAR(FROM_UNIXTIME(UPDATETIME),'YYYYMMDD')='".$_GET['Date']."' ORDER BY LOGID DESC)) WHERE RNUM>{$start} and RNUM<{$end}";
    }
    else
    {
        $query = "SELECT * FROM ( SELECT LOGID,TERMNO,PINSERIAL,TO_CHAR(UNIXTS_TO_DATETIME(UPDATETIME),'YYYY-MM-DD HH24:MI:SS'),CARDNO,CATEGORY,TRACENO,RESPONSECODE,SUBSTR(ORIGINALDATA,5,6),ACTIONID,ROWNUM AS RNUM  FROM ( select * from tbllogs WHERE TBLLOGS.TERMNO='".$term."' and TO_CHAR(FROM_UNIXTIME(UPDATETIME),'YYYYMMDD')=TO_CHAR(SYSDATE,'YYYYMMDD') ORDER BY LOGID DESC)) WHERE RNUM>{$start} and RNUM<{$end}";
    }
}
else if ( $card != "" )
{
    $query = "SELECT * FROM ( SELECT LOGID,TERMNO,PINSERIAL,TO_CHAR(UNIXTS_TO_DATETIME(UPDATETIME),'YYYY-MM-DD HH24:MI:SS'),CARDNO,CATEGORY,TRACENO,RESPONSECODE,SUBSTR(ORIGINALDATA,5,6),ACTIONID,ROWNUM AS RNUM  FROM (select * from tbllogs WHERE CARDNO='".$card."' ORDER BY LOGID DESC)) WHERE RNUM>{$start} and RNUM<{$end}";
    exit( );
}
else if ( $serial != "" )
{
    $query = "SELECT * FROM ( SELECT LOGID,TERMNO,PINSERIAL,TO_CHAR(UNIXTS_TO_DATETIME(UPDATETIME),'YYYY-MM-DD HH24:MI:SS'),CARDNO,CATEGORY,TRACENO,RESPONSECODE,SUBSTR(ORIGINALDATA,5,6),ACTIONID,ROWNUM AS RNUM  FROM (select * from tbllogs WHERE PINSERIAL='".$serial."' ORDER BY LOGID DESC)) WHERE RNUM>{$start} and RNUM<{$end}";
}
