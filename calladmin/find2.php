...
if ( isset( $_GET['Export'] ) )
{
    ...
    if ( $where_c != "" )
    {
        $result = run_my_query( $conn, "SELECT PINID,PINSERIAL,CHARGEFLAG,FROM_UNIXTIME(UPDATETIME),TRACENO,TXNDATE,RRN,FROM_UNIXTIME(TBLBATCH.BATCHDATE),TBLCATEGORY.NAME,CARDNO,TERMNO,TBPINS.AMOUNT,TBPINS.CATEGORY,ROWNUM AS RNUM FROM (SELECT * FROM tblpinsold_13940114 WHERE {$where_c} AND CHARGEFLAG=1 ORDER BY UPDATETIME DESC) TBPINS LEFT JOIN TBLBATCH ON TBPINS.BATCHID=TBLBATCH.BATCHID LEFT JOIN TBLCATEGORY ON TBLCATEGORY.CATEGORY=TBPINS.CATEGORY ORDER BY UPDATETIME DESC" );
    }
    while ( $row = oci_fetch_array( $result, OCI_RETURN_NULLS ) )
    {
        echo "\"{$row['0']}\",\"{$row['1']}\",\"{$row['2']}\",\"{$row['3']}\",\"{$row['4']}\",\"{$row['5']}\",\"{$row['6']}\",\"{$row['7']}\",\"{$row['8']}\",\"{$row['9']}\",\"{$row['10']}\",\"{$row['11']}\",\"{$row['12']}\"\n";
...
else
{
    ...
    if ( isset( $_GET['TermNo'] ) )
    {
        ...
        oci_free_statement( $stid );
        $stid = run_my_query( $conn, "SELECT to_char(to_date(CAPTUREDATE,'yyyymmdd'),'yyyy-mm-dd'),CATEGORY,COUNT(1),SUM(AMOUNT),MIN(UPDATETIME) FROM tblpinsold_13940114 WHERE CAPTUREDATE>=to_char(sysdate-{$total_day},'yyyymmdd') AND TERMNO='".$_GET['TermNo']."' AND CHARGEFLAG=1 GROUP BY CAPTUREDATE,CATEGORY" );
        $i = 0;
        while ( $row = oci_fetch_array( $stid, OCI_RETURN_NULLS ) )
        {
            $m_date = $row[4];
